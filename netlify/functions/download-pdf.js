...
async function generatePDF(relatorio) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // NOTA DE RODAPÉ (fixa no rodapé da página A4, mas sem forçar segunda página)
      const noteY = 750; // Posição fixa no rodapé (~90px do fim da página A4)
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Adicionar loja ao relatório (da primeira deslocação, pois todas têm a mesma loja)
    if (todasDeslocacoes.length > 0) {
      relatorio.loja = todasDeslocacoes[0].loja;
    }
